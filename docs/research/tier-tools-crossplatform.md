...
2. **eza** (Low) — no macOS prebuilt release asset exists upstream at all
   (verified against the v0.23.5 release asset list); relies on the
   non-aqua `vfox` backend on macOS specifically. Needs a real macOS runner check that
