...
import { AgentenvConfig, BINARY_MAP, TOOL_DESCRIPTIONS, TOOL_TIERS, DEFAULT_CONFIG } from '../config/schema.js';
...
export function generateAgentsMd(config: AgentenvConfig): string {
  const lines: string[] = [];
  const markerStart = config.generate?.marker_start || DEFAULT_CONFIG.generate?.marker_start || '<!-- agentenv-managed-start -->';
  const markerEnd = config.generate?.marker_end || DEFAULT_CONFIG.generate?.marker_end || '<!-- agentenv-managed-end -->';
  ...
  if (config.rtk?.enabled) {
    ...
    lines.push('');
    lines.push('- **rtk**: CLI proxy that reduces LLM token consumption by 60-90% on common dev commands');
    lines.push('  - Automatically rewrites commands like `grep`, `find`, `cat`, etc. to faster, more token-efficient alternatives');
  ...
  if (config.rtk?.enabled) {
    ...
    lines.push('- Output is truncated to reduce token usage while preserving essential information');
  ...
  lines.push('');
  lines.push('- This repository has been configured with agentenv for optimal AI coding agent performance');
 ...
 */
export function generateClaudeMd(agentsMdPath: string = 'AGENTS.md'): string {
  const lines: string[] = [];
...
export function generateInstructionFiles(
  ...
  const claudemdContent = generateClaudeMd('AGENTS.md');
  files.push({
    path: claudemdPath,
...
export function updateWithMarkers(
  ...
  try {
    ...
    const newEndIndex = newContent.lastIndexOf(markerEnd);
